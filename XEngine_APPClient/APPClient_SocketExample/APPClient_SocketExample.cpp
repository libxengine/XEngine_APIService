...
//linux::g++ -std=c++17 -Wall -g APPClient_SocketExample.cpp -o APPClient_SocketExample.exe -L /usr/local/lib/XEngine_Release/XEngine_BaseLib -L /usr/local/lib/XEngine_Release/XEngine_NetHelp -lXEngine_BaseLib -lNetHelp_APIClient
//macos::g++ -std=c++17 -Wall -g APPClient_SocketExample.cpp -o APPClient_SocketExample.exe -lXEngine_BaseLib -lNetHelp_APIClient
...
int test_create()
{
	int nLen = 0;
	int nCode = 0;
	LPCXSTR lpszAPIUrl = _T("http://127.0.0.1:5501/api?function=socket&params1=0");
...
int test_delete()
{
	...
	LPCXSTR lpszAPIUrl = _T("http://127.0.0.1:5501/api?function=socket&params1=1");
	if (!APIClient_Http_Request(_T("POST"), lpszAPIUrl, st_JsonRoot.toStyledString().c_str(), &nCode, &ptszMsgBuffer, &nLen))
	{
		printf("发送投递失败！\n");
		return 0;
